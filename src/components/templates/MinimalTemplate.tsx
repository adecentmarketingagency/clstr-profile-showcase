...
import {
  MapPin, Mail, ExternalLink, Sun, Moon,
} from "lucide-react";
    ...
    <div className={`min-h-screen ${bg} transition-colors duration-300`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
        ...
        <motion.footer variants={fadeUp} initial="hidden" animate="visible" custom={sectionIndex} className={`mt-16 pt-8 border-t text-center ${borderFooter}`}>
          <a href="https://clstr.in" target="_blank" rel="noreferrer" className={`text-xs ${textFaint} hover:${dark ? "text-white/40" : "text-gray-500"} transition-colors`}>Built with Clstr</a>
        </motion.footer>
      </div>
    </div>
  );
