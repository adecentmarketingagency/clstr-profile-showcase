...
import { Education, Experience, Project, Post } from "@/types/profile";
...
export default function ProfileEditor() {
  ...
  const addPost = () => {
    ...
    updateProfile({ posts: [...profile.posts, newPost] });
  };
  ...
  const removePost = (id: string) => {
    updateProfile({ posts: profile.posts.filter((p) => p.id !== id) });
  };
        ...
        >
          <div>
            ...
            <p className="text-white/50 text-sm mt-1">
              Your portfolio updates automatically as you edit.
            </p>
          </div>
            ...
            <GlassCard>
              ...
              <div className="space-y-4">
                {profile.posts.map((post) => (
                  <div key={post.id} className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06] space-y-3">
                    <div className="flex justify-between items-start">
                      <Input
                        value={post.title}
                      ...
                      />
                      <button onClick={() => removePost(post.id)} className="text-white/30 hover:text-white/60 p-1">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                    <Textarea
                      ...
                      className="bg-white/[0.04] border-white/[0.06] text-white/80 text-sm min-h-[100px]"
                      placeholder="Write your post..."
                    />
                  </div>
                ))}
                <Button onClick={addPost} className="bg-white/[0.06] border border-white/10 text-white/70 hover:bg-white/10 gap-2 w-full">
